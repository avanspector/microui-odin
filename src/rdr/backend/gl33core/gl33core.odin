...
draw_from_command_buffer :: proc(cmd: Command_Buffer) {
	...
	if uniform_buffer, ok := cmd.uniform_buffer.?; ok {
		...
		BufferSubData(gl_buffer_usage[.UNIFORM], 
		              0, len(cmd.uniform_data), 
		              raw_data(cmd.uniform_data))
	}
	...
	if index_buffer, ok := cmd.index_buffer.?; ok {
		...
		BufferSubData(gl_buffer_usage[.INDEX], 
		              0, len(cmd.index_data), 
		              raw_data(cmd.index_data))
